// # Yearly price Helper
// Usage: `{{yearlyPrice}}`
// gets the yearly price of the tier associated with a given post context.
...
module.exports = function yaerlyPrice() {
  ...
  if (accessProductsList && accessProductsList.length > 0) {
    const tierData = accessProductsList.map((tier) => {
      let _data = {
        yearlyPrice: escapeExpression(tier.yearly_price),
    ...
    if (tierData.length === 1) {
      ...
      if (_currency === "USD" || _currency === "EUR") {
        output = `${CURRENCIES[_currency]}${tierData[0].yearlyPrice}`;
      } else {
        suffix = CURRENCIES[_currency];
        output = `${tierData[0].yearlyPrice} ${suffix}`;
      }
    }
  }
